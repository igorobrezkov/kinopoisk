...
const resizeMenu = (screen: number) => {
  ...
  if (screen <= 590) {
    ...
    if (entey !== null && authData.value) {
      entey.textContent = authData.value?.surname;
    }

    if (enteyD !== null && !authData.value) {
      enteyD.textContent = "Войти";
    }
  ...
  <nav class="header__nav">
    <ul class="header__list">
      <li class="header__item">
        <router-link to="/" class="header__link header__link--main">
          Главная
        </router-link>
      </li>
      <li class="header__item">
        <router-link to="/genres" class="header__link header__link--genres">
          Жанры
        </router-link>
      </li>
      <li class="header__item header__item--search" @click="searchMobileGo">
        <SearchHeader
          @search-close="(val) => (visClose = val)"
          @is-vis="(val) => (isVis = val)"
          @input-value="(val) => (visInput = val)"
          :closeClick="isVis"
          v-if="searchVis"
        />
        <CloseSearch
          class="header__search-close"
          v-show="visClose || visInput"
          @click="clearInput"
        />
      </li>
    </ul>
