  ...
  <section class="favorites">
    ...
    <ul class="favorites__list">
      <li
        ...
        :id="film?.id"
        @mouseover="visClose(film?.id)"
        @mouseout="hidClose(film?.id)"
        :class="{
          'top-10__item--no-image': !film.backdropUrl && !film.posterUrl,
        }"
        v-if="promised.data.value"
        v-for="film in arrFilms.data"
        :key="film?.id"
      >
        ...
        <span class="favorites__close" @click="delMyFavorites(film?.id)"
          ><CloseFavirite
        /></span>
      </li>
    </ul>
