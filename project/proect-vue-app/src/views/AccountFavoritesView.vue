 ...
 <section class="favorites">
   {{ setArrRavorites(promised.data.value) }}
     ...
     <ul class="favorites__list">
        <li class="favorites__item" :id="film?.id"  @mouseover="visClose(film?.id)" @mouseout="hidClose(film?.id)" :class="{'top-10__item--no-image': !film.backdropUrl && !film.posterUrl}" v-if="promised.data.value" v-for="(film) in arrFilms.data" :key="film?.id">
